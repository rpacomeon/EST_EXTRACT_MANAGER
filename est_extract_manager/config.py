...
class Config:
    ...
    # Fix for Streamlit Cloud: Remove duplicate directory names in path
    # Streamlit Cloud sometimes creates paths like /mount/src/project/project/file
    project_name = _PROJECT_ROOT.name
    parts = _PROJECT_ROOT.parts
    if parts.count(project_name) > 1:
        # Find first occurrence and use that as root
        first_idx = parts.index(project_name)
        _PROJECT_ROOT = Path(*parts[:first_idx + 1])
        ...
        def normalize_path(path_str: str) -> str:
            ...
            if path_str_lower.startswith('/mount/src/'):
                ...
                if project_name in parts:
                    ...
                    # Check if duplicate (next part is also project name)
                    if len(parts) > project_idx + 1 and parts[project_idx + 1] == project_name:
                        # Remove duplicate: keep first occurrence, skip second
                        fixed_parts = parts[:project_idx + 1] + parts[project_idx + 2:]
